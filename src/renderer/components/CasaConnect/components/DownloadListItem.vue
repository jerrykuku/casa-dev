    ...
    <div class="tr is-unselectable" :class="{'isCutting':getCardState(item)}">
      ...
      <div class="td " :class="{'action-td':!isDownloading}">
        ...
        <div class="mr-2 is-flex is-justify-content-flex-end is-flex-grow-1 ">
          ...
          <b-tooltip :label="$t('D-Open')" type="is-dark" v-if="!isDownloading" append-to-body>
            <span data-role="icon" class="action-icon icon " @click="openInFolder">
              <b-icon icon="folder-open-outline" size="is-small"></b-icon>
            </span>
          </b-tooltip>
...
export default {
  ...
  methods: {
    ...
    openInFolder() {
      ...
      const local_path = require('path').join(this.item.local_path, '/')
      shell.openPath(local_path)
    },
    remove() {
