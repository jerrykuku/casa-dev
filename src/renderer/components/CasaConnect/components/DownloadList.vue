 ...
 * @Description: 
 * @FilePath: \CasaOS-UI\src\components\CasaConnect\components\DownloadList.vue
-->
<template>
  <div class="is-flex-grow-1 is-flex-shrink-1 is-flex is-flex-direction-column is-relative download-area calc-height">
    ...
    <div class="content-head p-20-40 is-flex">
      ...
      <div class=" is-flex-shrink-0">
        <b-taglist>
          <b-tag class="primary-light-hover-btn download-tab-button" @click.self=" page='downloading'" :type="downTabActive" rounded>
            {{ $t('Downloading') }}
            <span class="d-badge">{{downloadingList.length}}</span>
          </b-tag>
          <b-tag class="primary-light-hover-btn download-tab-button" @click.self=" page='completed'" :type="completeTabActive" rounded>
            {{ $t('D-Completed') }}
          </b-tag>
        </b-taglist>
      </div>
      ...
      <div class="table-thead ">
        <div class="tr-wrapper ">
          <div class="tr is-flex">
            <div class="th">{{ $t('D-Name') }}</div>
            <div class="th">{{ $t('D-Size') }}</div>
            <div class="th action-td">{{ $t('D-Action') }}</div>
          </div>
        </div>
      </div>
      <!-- Table header End -->
