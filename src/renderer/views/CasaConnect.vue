  ...
  <div class="acquaintance-share is-flex ">
    <multipane @paneResize="onPanerResize">
      ...
      <div class="a-side-bar is-flex-shrink-0">
        <!-- Header Start -->
        <div class="header p-20-40 is-flex is-align-items-center" style="-webkit-app-region: drag">

          <b-dropdown aria-role="list" class="file-dropdown" animation="fade1" append-to-body :mobile-modal="false">
            <template #trigger>
              <b-tooltip :label="$t('More info')" type="is-dark" position="is-right">
                <div id="v-step-0" class="avatar size-44 is-flex-shrink-0 is-clickable" style="-webkit-app-region: no-drag">
          ...
          <div class="ml-4 is-flex-grow-1 has-text-left" style="-webkit-app-region: no-drag">
      ...
      <div class="a-main-content is-flex-grow-1 is-flex is-flex-direction-column" id="drag-area">
        <div class="header p-20-40 is-flex is-flex-shrink-0 " style="-webkit-app-region: drag">
            ...
            <transition name="fade-transform" mode="out-in">
              <div class="is-flex is-flex-grow-1 is-align-items-center" v-if="currentUser.token != ''" :key="currentUser.token">
                <b-tooltip type="is-dark" :label="'ID : '+currentUser.token" position="is-right">
                  <div class="avatar size-44 is-flex-shrink-0 badge success" style="-webkit-app-region: no-drag">
                    <avatar :username="currentUser.nick_name==''?'test':currentUser.nick_name" :size="44"></avatar>
                  </div>
                </b-tooltip>
                <div class="ml-4 is-flex-grow-1 has-text-left " style="-webkit-app-region: no-drag">
                  <h1 class="title is-5 mb-0 one-line">{{currentUser.mark==""?currentUser.nick_name:currentUser.mark}}</h1>
                  <h2 class="subtitle is-7 mt-0 mb-0 one-line has-text-grey">{{currentUser.profile}}</h2>
...
import DownloadList from '../components/CasaConnect/components/DownloadList.vue'
import GirdView from '../components/CasaConnect/components/GirdView.vue'
import ListView from '../components/CasaConnect/components/ListView.vue'
import AddFriendModal from '../components/CasaConnect/modals/AddFriendModal.vue'
import DownloadModal from '../components/CasaConnect/modals/DownloadModal.vue'
import ProfileModal from '../components/CasaConnect/modals/ProfileModal.vue'
import SettingsModal from '../components/CasaConnect/modals/SettingsModal.vue'

import HistoryButton from '../components/CasaConnect/components/HistoryButton.vue'
import MyInfoModal from '../components/CasaConnect/modals/MyInfoModal.vue'
import InitModal from '../components/CasaConnect/modals/InitModal.vue'
import UserContextMenu from '../components/CasaConnect/components/UserContextMenu.vue'
...
export default {
  ...
  data () {
    return {
      ...
      cancelRequest: null,
      downloadListLen: 0,
      wsUrl: `ws://localhost:${process.env.SERVER_PORT}/v1/notify/ws?token=${this.$store.state.Casa.token}`,
      steps: [
        {
          target: '#v-step-0',
  ...
  computed: {
    ...
    viewIcon () {
      return this.$store.state.Casa.isShareViewGird ? 'view-grid-outline' : 'format-list-bulleted'
    },
    listView () {
      return this.$store.state.Casa.isShareViewGird ? 'gird-view' : 'list-view'
    },
    isNoneUserSelect () {
  ...
  methods: {
    ...
    **************************************************/
    ...
    getMyFriendList (showFirst = false) {
      this.$api.person.getFriendsList().then(res => {
        ...
        if (showFirst) {
          let user = find(this.myFriendsList, (o) => { return o.on_line == true })
          if (user !== undefined) this.getUserFiles(user, this.rootPath)
        }
      })
    },
    ...
    **************************************************/
    ...
    // Change View
    changeView () {
      this.isViewGird = !this.$store.state.Casa.isShareViewGird
