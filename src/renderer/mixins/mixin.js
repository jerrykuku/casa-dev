...
export const mixin = {
  ...
  methods: {
    ...
    getFileUrl (item) {
      let base_url = (process.env.NODE_ENV === 'development') ? `http://${this.$store.state.Casa.devIp}:${this.$store.state.Casa.devPort}/v1/file/new/download?` : `http://${document.location.host}/v1/file/new/download?`
      let url = {
        path: item.path,
        token: this.$store.state.Casa.token
      }
    ...
    /**
         ...
         */
    getCardState (item) {
      if (this.$store.state.Casa.operateObject != null && this.$store.state.Casa.operateObject.type == 'move') {
        return this.$store.state.Casa.operateObject.from == item.path
      } else {
        return false
      }
    }
